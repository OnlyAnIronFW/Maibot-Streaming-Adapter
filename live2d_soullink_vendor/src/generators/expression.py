...
class ExpressionGenerator(BaseGenerator):
    ...
    def _extract_json(content: str) -> dict:
        ...
        json_match = re.search(r"\{[\s\S]*\}", normalized_content)
        if not json_match:
            preview = ExpressionGenerator._content_preview(normalized_content)
            raise ValueError(f"无法解析 LLM 返回的 JSON(no_object): {preview}")
        candidate = json_match.group()
        try:
            return json.loads(candidate)
        except json.JSONDecodeError as exc:
            preview = ExpressionGenerator._content_preview(candidate)
            raise ValueError(f"无法解析 LLM 返回的 JSON(invalid_object): {preview}") from exc

    def _resolve_thinking_payload(self) -> dict | None:
        raw_value = getattr(self.config, "enable_thinking", None)
        ...
        if "deepseek" not in provider and "deepseek" not in base_url and not model.startswith("deepseek"):
            return None
        return {"type": "enabled" if bool(raw_value) else "disabled"}
    ...
    def _build_request_body(
        ...
        *,
        ...
        if bool(getattr(self.config, "response_format_json_object", True)):
            request_body["response_format"] = {"type": "json_object"}
        thinking_payload = self._resolve_thinking_payload()
        if thinking_payload is not None:
            request_body["thinking"] = thinking_payload
        return request_body
...
#1. 只生成非嘴部参数，禁止输出任何 Mouth 相关参数
    ...
    async def generate_motion_plan(
        ...
        request_body = self._build_request_body(
            system_prompt=self._generate_motion_plan_prompt(),
            user_message=user_message,
            max_tokens=self.config.max_tokens * 2,  # ?????? token
        )
    ...
    async def generate_tts_motion_frame(
        ...
        request_body = self._build_request_body(
            system_prompt=self._generate_tts_motion_prompt(frame_duration_ms),
            user_message=user_message,
        )
